...
export const useGetProductsBranchId = (branchId) => {
  return useQuery({
    queryKey: ["productsByBranch", branchId],
    queryFn: async () => {
      try {
        const { data } = await getActiveProductsByBranchId(branchId);
        return data;
      } catch (error) {
        console.log(error);
        throw error;
...
// GET Active Products by branch ID
const getActiveProductsByBranchId = async (branchId) => {
  return await productApi.get(`/tenant/product/branch/${branchId}/active`);
};
